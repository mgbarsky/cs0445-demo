// CS 0445 Spring 2023.  Demonstration of Java classes and objects,
...
public class Example3
{
      public static void main(String [] args)
      {
             ...
             for (int i = 0; i < 6; i++)
                 System.out.println(P[i].toString());  // This will implicitly
                                            // call the toString()
             ...
             SortArray.selectionSort(P, P.length);  // call sorting function
					...
					// implemented in Listing 15-1 from Chapter 15
					...
					// header (and its generic implications)

			 for (People pp: P)  			// Use iterator version of for loop
			 	 System.out.println(pp);	// to print data.  We will look at
			 	 							// iterators later this term.
             System.out.println();

             // Sorting method works for array of any Comparable objects.
             // Primitive types are not Comparable, but their wrapper classes
             // are.  This works due to polymorphism.  Within the
             // selectionSort() method, the code for compareTo() that is executed
             // is based on the objects being stored in the array.

             Integer [] intarray = new Integer[20];
             for (int i = 0; i < intarray.length; i++)
                      intarray[i] = Integer.valueOf((7*i)%(intarray.length));
             for (int i = 0; i < intarray.length; i++)
